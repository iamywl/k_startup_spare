...
| pie | ✅ | `pie1~12` 흰색 강제 |
| **xychart-beta** | ❌ | **표준 themeVariables 가 plot 라인 색을 제어 못 함 → 기본 팔레트(주황·파랑)로 렌더.** init 의 `themeVariables` 안에 아래 `xyChart` 블록을 **추가**해야 흑백이 된다(mmdc 렌더로 확인 완료, 2026-06-26). |
| **mindmap** | ❌ | **init 을 안 따름(회색 노드). → `flowchart` 로 변환**한다(root→카테고리→leaf). |

> **xychart-beta 흑백 처방** — `themeVariables` 객체 안(예: `"pie12":"#ffffff"` 바로 뒤)에 추가:
> ```json
> "xyChart":{"backgroundColor":"#ffffff","titleColor":"#000000","xAxisLabelColor":"#000000","xAxisTitleColor":"#000000","xAxisTickColor":"#000000","xAxisLineColor":"#000000","yAxisLabelColor":"#000000","yAxisTitleColor":"#000000","yAxisTickColor":"#000000","yAxisLineColor":"#000000","plotColorPalette":"#000000,#000000,#000000,#000000,#000000,#000000"}
> ```
> `plotColorPalette` 는 plot 계열 수만큼 `#000000` 을 콤마로 나열(다계열도 전부 검정 — 구분은 마커/선형 아닌 라벨로). bar/line 모두 적용됨.
