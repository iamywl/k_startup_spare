...
| 주관기관 | 대구대학교 창업지원단 |
| 트랙 | 실전창업[^15] |
| 지원금 | 기본 300만 원 · 최대 1,000만 원[^15] |
| 모집기간 | 2026-03-19 ~ 2026-04-02[^15] |
...
**첫 100명 — 채널별 도달→가입 산식 ([추정])**

| 채널 | 도달(회×규모) | 가입 전환 가정 | 가입 |
|:---|:---|:---:|:---:|
| 창업지원단·창업허브 세미나 현장 시연 | 3회 × 40명 = 120명 도달 | 35%(설치 0·현장 즉시 가입) | 약 42명 |
| 운영진 1:1 핸즈온(슬랙·오픈카톡 DM) | 80명 컨택 | 40% | 약 32명 |
| 세무·노무·보육센터 제휴 1곳 시범 안내 | 200명 노출 | 12% | 약 24명 |
| **합계** | | | **약 98명 ≈ 100명** |

> 첫 100명은 **유료 광고 없이** 세미나(고전환·소규모) + 직접 핸즈온 + 제휴 1곳으로 도달한다. 세미나당 약 14명 가입(40명×35%)이 핵심 엔진이다.

**첫 1,000명 — 오가닉·바이럴·제휴 누적 산식 ([추정])**

| 채널 | 12개월 누적 도달 | 가입 전환 가정 | 가입 |
|:---|:---|:---:|:---:|
| 세미나 확대 | 누적 20회 × 40명 = 800명 | 35% | 약 280명 |
| 콘텐츠 SEO(블로그·쇼츠 "이 조항 위험한 이유") | 월 2,000 유입 × 12 = 24,000 | 1.5% | 약 360명 |
| "내 계약서 위험점수" 공유 챌린지(바이럴) | 가입자 1인당 0.3명 추천 × 누적 base | — | 약 200명 |
| 세무·노무·보육센터 제휴 3곳 번들 | 누적 1,600명 노출 | 10% | 약 160명 |
| **합계** | | | **약 1,000명** |

> 1,000명 구간은 콘텐츠 SEO(저CAC 오가닉)와 챌린지 바이럴이 세미나의 인력 한계를 보완한다. 바이럴 계수 0.3은 보수적 가정이며 챌린지 성과로 보정 **[재확인 필요]**.
...
> **가격 근거(해외 경쟁 SaaS 벤치마크):** 해외 AI 계약 검토 소규모/개인 플랜은 LegesGPT $19.99~$69.99/월, Legly Professional $29~39/user·월(프리미엄), Signeasy AI $20/user·월 수준이다[^14]. 리걸체크 Pro(9,900~19,000원 ≈ $7~14)는 이들 **해외 entry 플랜보다 의도적으로 낮게** 잡아 무소비층(소상공인) 진입 마찰을 줄였고, Team(39,000~79,000원 ≈ $28~57)은 Legly·Signeasy의 user당 단가와 정합한다. 단, 환각 없는 룰엔진·국내 법령 특화라는 차이가 있어 절대 가격은 **[추정]**이며 베타 결제 실험으로 확정한다 **[재확인 필요]**.
...
> **산식(모두 [추정]):** 12개월 차 **누적 가입(무료 포함) = 1,000명**(§GTM "첫 1,000명" 목표를 매출 기준선으로 채택)을 base로 두고, 위 단위경제성표의 **ARPU 13,000원/월**과 시나리오별 **유료 전환율**을 곱한다.
> - **12개월 차 유료 사용자 = 1,000 × 전환율**
> - **월 MRR = 유료 사용자 × ARPU(13,000원)**
> - **12개월 누적 매출 ≈ 월말 MRR × 12 × 0.5**(0→1,000명 선형 증가 가정 시 평균 가입이 최종치의 약 절반 → ramp 계수 0.5)

| 시나리오 | 유료 전환 | 12개월 차 유료 사용자 | 12개월 차 MRR | 12개월 누적 매출 | 전제 |
|:---|:---:|:---:|:---:|:---:|:---|
| 보수 | 2% | 20명 | 약 26만 원 | 약 156만 원 | 오가닉만, 제휴 부진 |
| 기본 | 5% | 50명 | 약 65만 원 | 약 390만 원 | 제휴 2~3곳 가동 |
| 공격 | 9% | 90명 | 약 117만 원 | 약 702만 원 | 제휴 확장 + 챌린지 바이럴 |

> 위 수치는 모두 **[추정]**이다(ARPU·전환율·base 가입 수 미검증). 12개월 시점의 **연환산 매출 잠재력(ARR ≈ MRR×12)**은 보수 약 312만 원 → 공격 약 1,404만 원 구간이며, 이는 **유료 base 1,000명 기준의 1년차 초기 트랙션**이다. 가입 base가 다음 해 10배(1만 명)로 확대되면 같은 전환율에서 매출도 선형으로 약 10배가 된다. 정밀 값은 베타 결제 실험으로 확정한다 **[재확인 필요]**.
...
- **Why now:** 창업·소상공인 수가 누적되며[^1][^3] 다빈도 소액 계약이 폭증했고, 클라이언트 사이드 룰엔진으로 **서버 비용 없이** 즉시 검토를 제공할 기술 환경이 갖춰졌다. 국내외 리걸테크 시장이 계약 검토 자동화 수요로 빠르게 성장하는 한편, 그 솔루션 대부분이 로펌·대기업 법무팀을 겨냥해 **중소기업·소상공인의 법률 접근성 격차는 오히려 방치**되어 있다[^9] — 리걸체크는 이 무소비 하단을 정조준한다. LLM이 대중화되며 "자동 계약 검토"에 대한 수용도도 높아졌다.
...
|:---|:---|:---|:---|
| **검토 비용** | 변호사 자문 건당 수십만 원대[^12] | 무료~월 1.3만 원대 | **−20~50만 원/건** [추정], 다빈도일수록 격차 폭증 |
...
[^9]: 대한변호사협회·국내외 리걸테크 동향 보고서. 계약 검토 자동화 수요 증가·중소기업/소상공인의 법률 접근성 격차. [재확인 필요]. https://www.koreanbar.or.kr
...
[^14]: 해외 AI 계약 검토 SaaS 공개 가격(2026). LegesGPT $19.99/$34.99/$69.99·월, Legly Professional $29~39/user·월(Freemium), Signeasy AI $20/user·월, ContractCrab $30/120건. https://www.legesgpt.com/blog/best-ai-contract-review-software · https://agenticcontractreview.com/pricing-models/ · https://signeasy.com/blog/business/ai-contract-review-software
[^15]: 대구대학교 창업지원단 「2026년 창업동아리 지원사업(실전창업)」 공고. 트랙·지원금(기본 300만 원·최대 1,000만 원)·모집기간 출처. 공고 PDF 원본.
