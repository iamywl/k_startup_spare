...
| 인증 1사이클당 실무자 투입 man-day | <TODO: §17 인터뷰 실측>, 잠정 `[추정]` 40~80 man-day | 미검증 `[추정]` |
| 심사 직전 증적 재취합 공수 | <TODO: §17 인터뷰 실측> | 미검증 `[추정]` |

> 위 baseline 위에 가드레일 도입 후 목표 절감률(준비기간 X%↓·재취합 공수 Y%↓)을 얹어 'before→after'를 §7.3·§16에서 산출한다. 실무자 man-day는 출처가 없어 `[추정]`이며, 이 값이 확정되기 전까지 ROI 시간절감 주장은 가설임을 명시한다.
...
| **SOM (3년)** | SAM의 약 15~25% (자사 구독 기준 금액 점유) | **약 50~100억원 ARR** `[추정]` | ARPA·점유율은 가설. 도달 가능성은 §6.5 퍼널로 역산, Vanta·Drata 초기 궤적 벤치마크[^12][^13] |
...
|:---|---:|:---:|---:|:---|
| 보수 | 63억 | 3배(매출) | 약 189억 | 국내 IT서비스·보안 M&A 통상 3~5배 매출[^재확인] |
| 기준 | 63억 | 5배(매출/ARR) | 약 315억 | 국내 SaaS 코스닥 상장사 PSR 중간대 `[재확인 필요]` |
| 공격 | 100억(확장) | 8배 | 약 800억 | 고성장 SaaS 상단(국내 기준) |
| (참고만) 글로벌 상단 | — | Vanta 약 40배 ARR[^12] | — | **미국 정점기·글로벌 리더 멀티플, 자사 적용 금지** |

> **변경점**: 종전 기준 8배·공격 12~15배(Vanta 40배 앵커링)를 **한국 실거래 현실(보수 3배·기준 5배·공격 8배)** 로 하향했다. Vanta 40배는 본문 적용에서 빼고 '글로벌 상단 참고'로만 각주 강등한다. 국내 보안/SaaS 실거래 comparable 3~5건은 제출 전 `[재확인 필요]`로 확보한다.
...
> 모든 각주는 [`5_research/README.md`](./5_research/README.md) §5 각주의 1차/공공/정부/언론/글로벌 리서치·VC DB 출처와 연결된다.
...
- **`[재확인 필요]`**: SAM 사당 평균 지출·컴플라이언스 귀속분, 보안성지속서비스 proxy(별개 카테고리)는 KISIA 공식 분해 통계 부재로 재확인 권장. 동적 모수 증가율(§3.2(C))은 정책 효과 가정. **한국 SaaS/보안 M&A·코스닥 PSR 실거래 comparable(§6.7)은 0건 — 제출 전 3~5건 확보 필요**. 국내 CSP(NCP·KT) 자동수집 API 가용성(§10.3), 특허 선행기술·출원 우선일(§10.4), 코스닥 기술특례 요건(§6.7(D)), 정부 환수·인건비 계상 규정(§13·§14)은 공고·고시 원문으로 재확인. 제도 개편 세부([^9])는 공단·KISA 고시 원문 재확인. 글로벌 GRC 시장([^15])은 부분검증.
...
- (제출 전 필수) SAM 사당 평균 지출·proxy 공식 통계 / 한국 SaaS·보안 M&A·코스닥 PSR comparable 3~5건(§6.7) / 국내 CSP 자동수집 API 가용성(§10.3) / 특허 선행기술·출원 우선일(§10.4) / 코스닥 기술특례 요건(§6.7) / 제도 개편 고시 원문
-->
