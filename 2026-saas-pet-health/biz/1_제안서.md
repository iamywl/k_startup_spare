...
- **누락 방지(must)**: 견종 선택 → 프로토콜 자동 시드 → D-day 자동계산·완료 시 재계산·알림톡 발송 로그 ([`v3.html`](../projects/pet-health/v3.html) 스케줄·연동 뷰).
- **청구 통합(must)**: 진료 타임라인 → 청구서 자동 구성 → PDF → 제출→심사→승인 상태추적 (보험청구 뷰).
- **조기발견(세그먼트 must)**: 체중 이상치 탐지·건강 위험 스코어·**v3 질병 위험 예측(로지스틱)**·성장곡선 percentile (건강 리포트·예측 뷰).
- **입력 마찰 제거**: 수의사 진료기록 자동 반영·**v3 EMR 연동·웨어러블 OAuth 동기화**로 보호자 직접 입력 없이 데이터 축적.
...
[^7]: KB금융지주 경영연구소, 「2023 한국 반려동물 보고서」, 2023. 연간 동물병원 지출·질병 경험률(조기발견 가치 분모). `[재확인 필요]`
[^8]: 보험연구원·손해보험업계, 펫보험 청구 행태, 2023~2024. 청구 서류 부담·소액 청구 포기. `[재확인 필요]`
[^9]: 반려동물 예방접종·정기검진 준수 조사(KCI 학술·민간 리포트 종합), 2022~2024. 접종·구충 미준수·누락 비율. `[재확인 필요]`
