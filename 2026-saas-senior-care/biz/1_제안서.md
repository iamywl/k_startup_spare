...
대한민국은 이미 초고령사회에 진입했다. 통계청 고령자 통계 기준 65세 이상 인구는 1,051.4만 명으로 전체의 20.3%를 넘어섰고[^1], 통계청 장래인구추계는 이 비율이 2036년 30.9%에 이를 것으로 본다[^2]. 그 결과 노인장기요양보험 수급 인정자는 매년 증가해 2023년 110만 명을 넘어선 데 이어 2024년 116.5만 명(+6.1%)에 달했으며[^3], 이들을 돌보는 장기요양기관(요양원·주야간보호·방문요양 등)도 전국 29,058개소(재가 78.2%·시설 21.8%)로 늘었다[^4].
...
- **청구 손실·반려:** 장기요양 급여비용은 국민건강보험공단에 정해진 처리기준에 따라 청구해 지급받는다[^5]. 제공한 서비스가 기록·코드와 정확히 맞지 않으면 심사 과정에서 조정·반려·환수가 발생한다. 부당청구는 신고포상금(최대 2억) 대상이자 환수·행정처분 사유가 된다[^5b]. 월말에 한 달치 종이 일지를 보고 청구 코드를 역산하는 방식은 **누락(청구 못 한 서비스 = 받지 못한 매출)**과 **오류(과청구 = 환수·행정처분 리스크)**를 동시에 키운다.
- **평가 감점:** 공단은 장기요양기관 정기 평가를 고시된 평가방법에 따라 실시하고 그 등급이 가산·수가·이용자 선택에 직결된다[^6]. 2024년 재가급여 평가에서 9,035개소 평균 81.1점, A등급 21.8%·E등급 10.7%로 등급이 크게 갈렸다[^6b]. 평가의 상당 부분이 **기록의 충실성**(돌봄계획·제공기록·욕구사정 등)을 본다. 기록이 종이에 흩어져 있으면 평가 준비에만 수 주가 들고, 빠진 기록은 그대로 감점이 된다.
- **보호자 신뢰 단절:** 입소 어르신의 가족은 부모가 어떻게 지내는지 거의 알지 못한다. "오늘 식사 잘 하셨나요"를 전화로 묻고, 기관은 그때그때 구두로 답한다. 사건(낙상·발열·전원)이 생겨야 비로소 연락이 오가는 **사후·수동 소통** 구조는 작은 오해도 분쟁으로 키운다. 노인학대·돌봄 품질에 대한 사회적 민감도는 계속 높아져, 2024년 학대피해노인은 7,167명(+2.0%)으로 집계됐다[^7].
...
소규모 시설이 "병원 EMR은 과하다"고 느낀 가격 장벽을 넘지 않도록, **월 구독액이 청구 1~2건 회수·평가 1등급 차이보다 작게** 설계한다. 아래 절대 금액은 **베타 가격 실험 전 자체 추정값**이며 공식 수치와 섞지 않는다(`[추정]`, [§데이터 정직성](#데이터-정직성-선언)).

| 티어 | 대상(정원) | 월 구독액(시설 단위) [추정] | 비고 |
|:---|:---|---:|:---|
| Basic | ~30인 | 99,000원 | 대시보드·돌봄기록·청구보조 |
| Standard | 30~50인 | 149,000원 | + 평가 리포트·일정/복약 |
| Pro | 50~70인 | 199,000원 | + 보호자 알림 채널·심화 분석 |
| B2B 라이선스 | 다기관 법인 | 기관당 단가 할인(별도 견적) | 프랜차이즈·법인 통합 |

기준 ARPA(시설당 월평균 구독액)는 **정원 50인 Standard 시설 = 149,000원/월(연 1,788,000원)**으로 잡는다(이하 단위경제성 계산의 기준값). [추정]

### R-3. 단위경제성 (시나리오 1세트 — 전부 [추정])

아래 절대값은 **베타 실측 전 자체 추정**이며, 위 기준 ARPA(149,000원/월)와 SaaS 표준 가정에서 산식으로 도출했다. 검증된 외부 수치와 섞지 않는다([§데이터 정직성](#데이터-정직성-선언)). 가정은 셀에 명시했다.

| 지표 | 산식 | 값 [추정] |
|:---|:---|---:|
| ARPA | 기준 티어 월 구독액 | 149,000원/월 |
| 기여이익률 | SaaS 한계비용 낮음 가정 | 80% |
| 월 기여이익 | ARPA × 기여이익률 = 149,000 × 0.8 | 119,200원/월 |
| 평균 유지개월 | 월 이탈률 2% 가정 → 1÷0.02 | 50개월 |
| **LTV** | 월 기여이익 × 유지개월 = 119,200 × 50 | **약 596만원** |
| **CAC** | 콘텐츠·레퍼럴 혼합(영업 인건비+제휴비) 가정 | **약 80만원/시설** |
| **LTV/CAC** | 596만 ÷ 80만 | **약 7.5배** (목표 3↑ 충족) |
| **회수기간** | CAC ÷ 월 기여이익 = 800,000 ÷ 119,200 | **약 6.7개월** |

**표 2.** 단위경제성 시나리오(기준 티어, 전부 [추정] — 베타 실측으로 대체). 핵심 가정: 월 이탈률 2%(청구·평가 반복 주기로 인한 높은 그로스 리텐션 가정), 기여이익률 80%, CAC 80만원/시설.

### R-4. 매출 시나리오 (3안, 절대값 — 전부 [추정])

24개월차 유료 시설 수 × 기준 ARPA(149,000원/월) 기준 연환산 ARR로 환산했다. 시설 수·전환율은 자체 추정이며 베타 실측으로 대체한다([추정], 산식 명시).

| 시나리오 | 24개월차 유료 시설 [추정] | 연환산 ARR(시설수 × 149,000 × 12) [추정] | 가정 |
|:---|---:|---:|:---|
| 보수 | 50개소 | 약 0.89억원 | 지역 한정·레퍼럴 위주 저속 확산, 협회 제휴 지연 |
| 기본 | 150개소 | 약 2.68억원 | 콘텐츠+제휴+레퍼럴 결합 정상 확산, 평가 시즌 도입 집중 |
| 공격 | 400개소 | 약 7.15억원 | 다기관 법인·프랜차이즈 라이선스 조기 체결로 견인 |

**표 2-1.** 매출 시나리오(24개월차 ARR, 전부 [추정]). 산식: 유료 시설 수 × 149,000원/월 × 12개월. 시설 수 절대값은 베타 가격·전환율 실측 후 확정한다(현 단계 창작 금지, [§데이터 정직성](#데이터-정직성-선언)).
...
- **Why now:** 초고령사회 진입[^1]·수급자 116.5만 돌파[^3]로 시설 수(29,058개소)[^4]와 행정 부담이 동시에 폭증하는데, 평가·청구의 디지털 정합 요구는 갈수록 강해진다[^6]. "종이로 버티기"의 한계가 임계에 왔다.
...
### ② 크기 정량화 (고객 언어의 월 금액 — 전부 [추정])

차별점이 만드는 가치를 시설장이 체감하는 **월 단위 금액**으로 환산한다. 아래는 정원 50인 시설 기준 **자체 추정**이며, 베타 기관 인터뷰·실측으로 검증해 `5_research/`에 추가한다(검증 전 `[추정]` 유지, 공식 수치와 혼용 금지).

- **청구 측(누수·반려 회복):** 시설이 매월 청구하는 급여비 중 종이 역산 과정에서 **누락·반려되는 비율을 1%만 줄여도** 회복 금액이 발생한다. 1인 월평균 급여비 약 150만원[^3b] × 정원 50인 = 월 청구 약 7,500만원 → 그 1% = **약 −75만원/월 손실 회복** [추정]. 자동 매핑·정합 점검으로 줄어드는 이 금액이 월 구독액(149,000원)을 **약 5배 상회**하면 구매는 ROI 양(+)이 된다.
- **평가 준비 측(인력시간 절감):** 평가철 기록 취합·정리에 사회복지사 1명이 **월 16시간(2일)** 추가 투입한다고 가정하면, 시급 약 1.5만원 기준 **약 −24만원/월** [추정]의 인건시간을 상시 적립으로 절감. 더해 등급 하락 시 잃는 가산·이용자 이탈은 별도 손실(미정량 [추정]).
- **합산 ROI(기준 시설):** 청구 회복 −75만 + 평가시간 −24만 ≈ **월 약 99만원의 손실/비용 회피** [추정] vs 구독 14.9만원 → **약 6.6배**. 이 배수가 전환 마찰을 넘는 "10배 규칙"에 근접한다(보수적으로도 5배+).
- **10배 규칙:** 위 효과의 합이 시설이 느끼는 전환·도입 마찰(교육·습관 변경)보다 충분히 커야 한다. 청구·평가가 **반복 주기**라 효과가 매월·매년 누적되므로 누적 가치가 마찰을 넘을 구조다. 위 절대값은 [추정]이며, 실증은 베타 KPI(반려율·준비시간)로 확인한다.

### ③ 외부 근거

위 주장의 토대(고령화·수급자 규모·1인 월평균 급여비·평가/청구 제도·부당청구 환수)는 모두 `5_research/`의 공식 출처[^1][^3][^3b][^5][^5b][^6][^6b]로 연결한다. **구매동인의 핵심 손실(반려·감점 금액)의 시설별 절대값(②의 −75만/−24만/−99만원)은 현재 자체 추정**이며, 1인 월평균 급여비 150만원[^3b]·부당청구 환수 제도[^5b]·평가 등급 분포[^6b] 같은 공식 수치를 토대로 시설 규모를 곱해 도출했다. 절대값은 베타 기관 인터뷰·실측으로 검증해 `5_research/`에 추가하며, 검증 전에는 `[추정]`을 유지하고 공식 수치와 섞지 않는다.
...
수급자 116.5만 명[^3]·전국 29,058개소 기관[^4]이라는 구조적 수요 위에, 중소 시설 구간의 미충족(그림 3)이 SOM의 출발점이다. 시장 규모의 **하향식(top-down) 추정 프레임**은 다음과 같다(절대 금액은 가격·전환율 실측 후 확정, [§데이터 정직성](#데이터-정직성-선언)).
  - **TAM:** 전국 장기요양기관 29,058개소[^4] × 운영관리 SaaS 연 구독액. 시설급여 기관(6,323개소)·주야간보호 등 시설형이 1차 타깃.
  - **SAM:** 전산 전담인력이 없는 정원 10~70인 중소 요양원·주야간보호(ICP). 본 구간은 비소비 상태(그림 3)로 미충족이 가장 크다.
  - **SOM:** 지역 거점(대구·경북 등)에서 무료 베타로 진입한 레퍼런스 시설군 → 협회·콘텐츠 확산.
...
> **수집 현황: 본 제안서 직접 인용 14건(공식·1차) / 통합 출처 352 / 목표 1,000 (목표 미달, 정직 표기).** 아래 각주는 제안서 본문이 직접 인용한 1차·공식 출처이며, 모두 [`5_research/README.md`](./5_research/README.md)의 검증된 항목(실 URL·발표연월·수치)과 1:1로 대응한다. 전체 근거 출처는 README에 **352건**(섹션 A~N)으로 통합돼 있고, 목표 1,000+ 는 추가 사이클로 누적 확보한다(현 진척 352/1,000도 정직 표기). 허위 충족·날조·중복 부풀리기 금지(`CLAUDE.md` §2.6).
>
> 통계·제도 인용(①~⑭ 중 통계청·NHIS·복지부·법령)은 README 각주 번호를 괄호로 병기했다. 경영이론 4종(파괴적 혁신·블루오션·린스타트업·JTBD)은 공인된 출판 저작이므로 서지를 직접 명시한다.

[^1]: **통계청 「2025 고령자 통계」** (2025.09). 65세+ 1,051.4만명·20.3%. (README §A #2) https://www.kostat.go.kr/board.es?mid=a10301010000&bid=10820&act=view&list_no=438832
[^2]: **통계청 「장래인구추계: 2022~2072년」** (2023.12). 65세+ 20.3%(’25)→30.9%(’36)→40%+(’50). (README §A #3) https://www.kostat.go.kr/board.es?mid=a10301010000&bid=207&act=view&list_no=428476
[^3]: **국민건강보험공단 「2024 노인장기요양보험 통계연보」** (2025.06). 인정자 116.5만명(+6.1%), 급여비 16조1,762억. (README §B #39) https://www.nhis.or.kr/nhis/together/wbhaec07200m01.do
[^3b]: **국민건강보험공단 「2024 통계연보 — 급여 유형」** (2025.06). 재가 62.6%·시설 37.3%, 1인 월평균 약 150만원. (README §B #42) https://www.bokjitimes.com/news/articleView.html?idxno=40325
[^4]: **국민건강보험공단 「2024 통계연보 — 기관 현황」** (2025.06). 장기요양기관 29,058개소(재가 78.2%·시설 21.8%). (README §B #41) https://www.rehabnews.net/news/articleView.html?idxno=23632
[^5]: **국가법령정보센터 「장기요양급여비용 청구 및 심사·지급업무 처리기준」** (admRulSeq 2100000074210). 청구·심사·지급·조정 처리기준. (README §C #70) https://www.law.go.kr/admRulLsInfoP.do?admRulSeq=2100000074210
[^5b]: **국민건강보험공단 「부당청구 장기요양기관·부정수급자 신고·포상금 규정」** (SEQ=177). 부당청구 신고포상금 최대 2억·환수. (README §C #95) https://www.nhis.or.kr/lm/lmxsrv/law/lawFullView.do?SEQ=177
[^6]: **국가법령정보센터 「장기요양기관 평가방법 등에 관한 고시」** (admRulSeq 2100000105629). 5개 영역·A~E 등급 평가, 가산·수가 연동. (README §C #89) https://www.law.go.kr/admRulLsInfoP.do?admRulSeq=2100000105629
[^6b]: **메디포뉴스 「2024 재가급여 평가결과」** (건보공단). 9,035개소 평균 81.1점, A 21.8%·E 10.7%. (README §C #94) https://medifonews.com/news/article.html?no=200069
[^7]: **보건복지부 「2024 노인학대 현황보고서」** (list_no=1486278). 학대피해노인 7,167명(+2.0%). (README §E #121) https://www.mohw.go.kr/board.es?mid=a10411010100&bid=0019&act=view&list_no=1486278
[^8]: **Christensen, C. M. 『The Innovator's Dilemma』** (Harvard Business Review Press, 1997). 파괴적 혁신(저가·비소비 고객에서 출발).
[^9]: **Kim, W. C. & Mauborgne, R. 『Blue Ocean Strategy』** (Harvard Business Review Press, 2005). ERRC 격자·가치 곡선 재설계.
[^10]: **Ries, E. 『The Lean Startup』** (Crown Business, 2011). MVP·검증된 학습(Build-Measure-Learn).
[^11]: **Christensen, C. M., Hall, T., Dillon, K. & Duncan, D. S. 「Know Your Customers' Jobs to Be Done」** (Harvard Business Review, 2016). JTBD 프레임워크.

---

### 데이터 정직성 선언

본 제안서의 모든 통계·제도 인용은 `[^n]` 각주로 표기하고 [`5_research/`](./5_research/) 출처와 연결한다 — 본문 직접 인용 14건은 전원 README의 검증된 항목(실 URL·발표연월·수치)에 1:1 매핑되며, 서지 미확정 stub은 0건이다(모든 각주가 확정 출처). 검증 전 자체 추정값(수익모델 R-2~R-4 절대 금액, 구매동인 ② −75만/−24만/−99만원 등)은 본문에 **`[추정]`**으로 명시했으며, 공식 수치와 한 문장에 섞지 않았다. 차별점 표(표 4)의 가치 수치 중 미검증분은 `[추정]` 또는 방향(↑/↓) 표기로만 두었다. 통합 근거 출처는 현재 **352건**으로 목표(1,000+)에는 미달하며, 이를 머리에 정직히 표기했다 — 수량을 위해 존재하지 않는 출처를 만들지 않는다. 미달분은 다음 사이클로 이월한다.
...
- 구매동인 ② 손실 절대값(−75만/−24만/−99만원) (현재 [추정] — 베타 기관 인터뷰로 검증)
- 통합 근거 출처 352 → 1,000+ 누적 수집 (research-collector 분산)
-->
