...
| 현장 워커 객단가(가정) | 1인 월 5,000원 `[추정]` (인접 협업툴 단가 근거) | [^18][^19][^20] |
...
> **객단가 가정의 취약성 (명시)**: 본 시나리오와 §3 TAM(약 1.38조)은 모두 "현장 워커 1인 월 5,000원" 단일 객단가에 의존한다. 이 단가는 **인접 사무직 협업툴 단가에서 차용한 가정**이며, 현장 워커의 실제 지불의사(WTP)는 아직 검증되지 않았다 `[추정]`. 위 표의 **객단가 민감도(하방)** 행은 Starter 단가를 3,000원으로 낮춰(블렌디드 ARPU −40%) 가정이 빗나갈 경우의 ARR 하한을 보인 것이다. TAM(1.38조)이 현존 국내 협업툴 시장(약 4,000~5,000억[^4])의 약 3배인 점도 이 단가 가정에 의존하므로, WTP는 §7 파일럿에서 **반드시 1차 검증 KPI로 측정**한다(아래 §7 KPI·§9 리스크 참조).
>
...
> 모든 각주는 [`5_research/README.md`](./5_research/README.md) 에서 2026-06-06 웹 검색·웹페치로 검증·정리한 출처와 연결된다.
