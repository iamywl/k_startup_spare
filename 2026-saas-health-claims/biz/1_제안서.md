...
# 제안서 — 디지털치료제·비대면진료 보험청구·환자 온보딩 자동화 SaaS 「클레임브릿지」
...
| 아이템 | 디지털치료제(DTx)·비대면진료 사업자·의원급 의료기관용 **보험청구·환자 온보딩 자동화 B2B SaaS 「클레임브릿지(ClaimBridge)」** |
...
> **포지션 정정(실사 핵심).** 본 제품은 심평원에 **직접 청구를 제출하는 주체가 아니다.** 요양급여비용 청구의 법적 주체는 **요양기관**이며, 청구는 요양기관업무포털·EDI 또는 **심평원 청구소프트웨어 검사·인증**을 거친 청구SW를 통해 이루어진다. 따라서 클레임브릿지는 "**청구 데이터 생성·전처리·검증 레이어**"로 포지셔닝하고, 실 제출은 (a) 요양기관이 보유한 인증 청구SW로 export, 또는 (b) 자체 청구SW 검사·인증 취득(리드타임은 [그림 6-b]·§13)으로 처리한다. 실손24도 임의 API가 아니라 **보험개발원 중계 인프라** 연동·자격이 전제이며 별도 확인 항목으로 분리한다([그림 6-b]). 이 전제·자격이 확정되기 전에는 제품 가치를 "**청구 전처리·반려 사전점검 도구**"로 보수적으로 한정한다(plan B, §9).
...
**EMR 연동 기술 전략(상호운용성).** 국내 EMR은 벤더별 폐쇄 포맷으로 파편화돼 있어 "1시간 셋업"은 표준 CSV 한정 수치다. 대응 전략은 3단계다. ① **표준 우선** — HL7 FHIR·진료정보교류 문서표준을 1차 채택하고 가능 벤더부터 표준 연동. ② **fallback** — 표준 미지원 EMR은 CSV 컬럼 매핑 마법사로 흡수. ③ **커넥터 우선순위** — 점유율 상위 EMR 벤더(유비케어·비트 등) `[추정]` 포맷을 조사해 우선 커넥터를 개발하되, 비표준 EMR은 **별도 커넥터 개발 리드타임**(건당 수일~수주)을 정직히 인정한다. EMR **직접 연동** 시 의료법 제23조의2(전자의무기록 인증) 적용 여부를 사전 검토하고, 직접 연동이 부담이면 **export-import 우회**(인증 부담 회피)를 기본값으로 둔다.

**환자 온보딩 fallback(의원 데스크 부담 최소화).** 환자 동의·전자서명·본인확인 자동화가 고령·디지털 취약 환자에게 오히려 데스크 업무를 늘릴 위험을 차단한다. ① 디지털 취약 환자용 **데스크 대행 모드**(직원이 태블릿으로 대리 진행), ② **종이 동의 병행**(스캔→이력 무결성 보장), ③ 온보딩 소요시간 목표 환자 1인 ≤ 90초 `[추정]`. "DTx 사업자 개별 부담 해소"(공급자 가치)뿐 아니라 "**의원 데스크 부담을 늘리지 않는다**"(구매자 가치)를 명시적 설계 목표로 둔다.

> **전환비용·lock-in을 구매자 언어로 재서술.** 전환비용(§5.3 2층)은 "데이터를 묶는다"가 아니라 **운영 통합**에서 나온다. raw 청구이력은 위 표대로 **언제든 표준 CSV/PDF로 전량 반출 보장**한다(이미 이탈 시 export 행). 옮겨가지 않는 자산은 룰 커스터마이즈·반려예측 학습상태·플랫폼/보험사 연동 설정·팀 운영 숙련이다. 즉 "들어오긴 쉽고, **데이터는 언제든 가져갈 수 있으며**, 다만 같은 운영 효율을 재구축하는 데 시간이 든다"가 정확한 표현이다. 최소계약기간·해지통보·위약금 조건은 §6.7 계약정책에 명시한다.
...
**왜 기존 강자가 "즉시"가 아니라 "12~18개월 늦게" 대응하는가 (해자의 정합성).** 기존 강자의 진입은 **기정사실**로 둔다(§5.1: 12~18개월). 우리가 사는 것은 "영원한 카피 불가"가 아니라 **그 12~18개월의 선행 격차**다. (a) **아키텍처 종속(지연 요인, 장벽 아님)** — 레몬헬스케어는 B2C 실손 파이프라인, EMR 사업자(유비케어 등)는 전통 EDI에 데이터 모델이 최적화돼 있어 신규 워크플로 단일 데이터화에 **재설계 시간**이 든다. 단 신규 사업부/별도 제품으로 출시하면 코드베이스 재설계가 결정적 장애는 아니므로 이를 **해자로 쓰지 않는다**. (b) **하방 확장 유인은 오히려 강하다(정직한 인정)** — 상급병원 시장이 포화되면 의원급·DTx로의 하방 확장은 강자에게 **자연스러운 다음 수순**이다. 따라서 "안 한다"가 아니라 "**아직 안 한, 늦게 할**" 것으로 본다. (c) **신규 제도 추적은 '비용 패리티' 항목** — DTx 수가 괴리[^5]·실손24 코드체계[^3]·비대면 청구 규정[^1] 룰엔진의 입력은 **공개된 심평원 수가·EDI 고시**라 누구나 본다. 이는 진입장벽이 아니라 **운영비용**이며, 진짜 비대칭은 §5.3-(2)의 **비공개 반려결과 데이터**에서만 나온다. 결론: 12~18개월 격차를 **선점 세그먼트의 전환비용·전용성**(§5.3 2·3층)으로 영구 우위 전환하는 것이 방어의 본질이다.
...
**(D) 멀티호밍 억제(3층 네트워크가 비독점 채널과 충돌하지 않게).** §7.1은 "채널 비독점"을 인정하므로, 양 사이드 독점이 아니라 **멀티호밍을 비싸게 만드는 메커니즘**으로 네트워크 효과를 설계한다: ① **DTx-보험사 정산 매칭**, ② **반려패턴 cross-tenant 학습**은 **단일 플랫폼에 통합될 때만** 가치가 나오고 분리 운영 시 가치가 급감하도록 설계. 이 메커니즘이 약하면 3층 난이도를 "높음 → 중"으로 하향하고 방어 무게를 2층(운영 통합 전환비용)으로 옮긴다.

**(E) export 보장과 2층 락인의 정합.** §2.4가 raw 청구이력 전량 export를 보장하므로 "데이터 보유" 자체는 락인이 아니다(자칫 import 마법사가 경쟁사에도 작동하는 양날의 칼). 따라서 2층 락인을 **데이터 보유 → 운영 통합 재구축 비용**으로 재정의한다(§2.4 박스 참조). churn ≤ 3% 목표는 "데이터가 묶여서"가 아니라 "**운영 효율 재구축에 시간이 들어서**" 달성된다.
...
**(B-2) churn ↔ NRR 정합(투심 지적).** 월 churn 3%는 **연 GRR 약 69%**(=0.97¹²)에 해당하므로, NRR ≥ 100%(§5.3)를 달성하려면 **연 +31%p 이상의 확장매출(업셀)** 이 필요하다. 확장 기여원: ① 청구 **건당 종량** 증가(볼륨 성장), ② **반려예측·최적화 리포트** 부가모듈, ③ **추가 기관(다지점)** 확장. 이 업셀이 GRR 하락분을 상쇄하지 못하면 **NRR 목표를 100% → GRR+α 현실치로 하향**한다(§11.3 KPI에 NRR/GRR 분리 표기). 즉 churn과 NRR을 한 표에서 정합화한다.
...
> **결론(예시): 월 15만원 대비 절감 약 49.5만원 → 페이백 약 1개월 미만, 연 환산 절감 약 595만원** `[추정]`. 입력은 파일럿 실측으로 교체(§11.4).
...
[^15]: **건강보험심사평가원·보건복지부 「건강보험통계연보(요양기관 종별 현황)」** (`[재확인 필요]`). 전국 의원급 의료기관 약 3.6만 개소·약국 약 2.5만 개소 규모. 본문 §3.1 모수 상한 산정에 사용하며, 정확 수치는 최신 통계연보·보건의료빅데이터로 확정한다. (출처 URL·연도 5_research 보강 예정)
