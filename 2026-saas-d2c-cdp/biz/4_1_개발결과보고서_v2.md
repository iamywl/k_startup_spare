...
#### 모바일 워크플로 전수 (iPhone 13, 390×844 · `captures/mobile/v2/`)

위 2컷에 더해, v2 핵심 워크플로를 모바일 뷰포트에서 실제 구동하여 전 구간 캡처했다 — 가로 overflow 0, 페이지 에러 0.

![모바일 대시보드](./captures/mobile/v2/01_dashboard.png)
- KPI 카드 스택 + 일자별 매출·광고비 라인 + 매체별 광고비 도넛 + Last Click 효율표. 하단 Bottom Nav 5탭 고정.

![모바일 어트리뷰션 — Markov](./captures/mobile/v2/02_attribution_markov.png) ![모바일 깔때기](./captures/mobile/v2/03_funnel.png)
- 어트리뷰션 모델을 Markov로 전환 → 매체별 기여 바차트·도넛·Last Click 대비 차이표가 모바일 단일 컬럼으로 재계산. 깔때기(퍼널) 뷰 스택 정상.

![모바일 세그먼트 빌더 모달](./captures/mobile/v2/04_segment_builder_modal.png) ![모바일 RFM](./captures/mobile/v2/05_rfm.png)
- 세그먼트 빌더 모달이 모바일 시트로 표시 — 이름·채널·등급·최소 결제액 입력 후 저장. RFM 자동 세그먼트 뷰.

![모바일 CSV 가져오기](./captures/mobile/v2/06_csv_import.png) ![모바일 매체 연동 OAuth 동의](./captures/mobile/v2/07_oauth_consent.png)
- CSV 업로드 → 미리보기. 네이버검색 OAuth mock 동의 화면이 모바일 시트로 표시.

![모바일 연동 완료](./captures/mobile/v2/08_integrations_connected.png) ![모바일 자동 보고 발송 로그](./captures/mobile/v2/09_schedule_sendlog.png)
- OAuth 허용 후 연동 완료 상태 반영. 자동 보고 스케줄러 Slack 테스트 발송 → 발송 로그 기록.
