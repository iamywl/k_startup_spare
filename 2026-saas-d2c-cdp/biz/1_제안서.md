...
> 단위경제: LTV ≈ 938만원 / 블렌디드 CAC ≈ 55만원 / **LTV:CAC ≈ 17배(보수 시나리오 3~6배)** / CAC payback ≈ 1.5개월(§6.3). 재무: 3개년 누적 유료 50→200→500 브랜드, **ARR 3억→12억→30억[추정]**, BEP ≈ Y2 말~Y3 초(§13). 고객 ROI: 월 광고비 3천만 브랜드 기준 절감액 ≈ 210만/월 vs 구독 49만 → **약 4.3배[추정]**(§17). 자금소요·고용·KPI·아키텍처·컴플라이언스·IP·Exit는 §13~§22 신설 절에 정량화.
>
...
| Customer Segments | 적합 활성 자사몰 D2C SMB 약 2만 [추정] | §3.2 SAM |
...
> §5.1~§5.5가 핵심 3축(한국 매체 연동·멀티터치 어트리뷰션·SMB 가격)을 *비교·방어 시나리오* 중심으로 다뤘다면, 본 절은 그 축을 **8개 카테고리(기술·데이터·어트리뷰션·운영·가격·GTM·네트워크효과·UX)로 전수 분해**해 차별점을 도출한다. 각 행은 *경쟁사 현황 → 우리 차별점 → 고객 가치* 형식이며, **핵심 항목은 구매동인(§차별화 기술의 구매동인 논증)으로 연결**된다.
>
> **정직성 원칙** — (1) 가치 수치 중 외부 공식 출처가 없는 것은 모두 `[추정]` 표기. (2) §5.5에서 밝혔듯 다수 차별점은 *현 시점 스냅샷*이며 T+12개월 복제 가능(영속성은 ○로 표기, ◎는 6개월짜리 일시 우위). (3) 사소·중복·억지 항목으로 50개를 부풀리지 않았다 — 카테고리별 의미 있는 축만 담았다.

**범례** — 영속성(경쟁사 복제 후에도 남는가): ◎ 강함 / ○ 중간 / △ 약함(일시 우위). 구매동인(§구매동인 논증 연결): ★ must-have급 / ☆ nice-to-have.

**표 5-6a. 기술·아키텍처 차별점 (8)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 1 | 글로벌 CDP는 미주 매체 SDK 중심, 한국 매체 커넥터 부재(§5.1) | 네이버·카카오·메타·구글·쿠팡 5종 OAuth 2.0 단일 파이프라인 | 5개 콘솔 수기 합산 제거(−약 4시간/주)[추정] | ○ | ★ |
| 2 | in-platform(Shopify)·앱 SDK 종속(Triple Whale·mParticle) | 자사몰 webhook(cafe24/shopify orders) HMAC 서명·멱등키 dedup 수신 | 주문 인입 실시간 반영, 중복 매출 집계 차단 | ○ | ☆ |
| 3 | 엔터프라이즈 데이터 웨어하우스 전제(Segment Business) | 단일 제품 경량 파이프라인(브라우저 자체완결 데모로 입증) | 인프라 셋업 0, TTV ≤1영업일(§2.4) | ○ | ★ |
| 4 | 쿠키리스 대응 미흡, 3rd-party 추적 잔존 | 1st-party·서버사이드(webhook) 수집 설계 | Chrome 철회·Safari 차단[^18] 환경에서 신뢰도 유지 | ○ | ☆ |
| 5 | 매체별 토큰 수명·동기화 상태 비가시 | OAuth 토큰 만료(3600s)·`syncedAt` 동기화 시각 노출 | 연동 끊김을 사용자가 즉시 인지 | △ | ☆ |
| 6 | 모바일 미대응·데스크톱 전용 콘솔 다수 | 390×844 반응형(바텀 탭바·스택 레이아웃, §모바일 대응) | 외근 중 모바일로 예산 결정 가능 | △ | ☆ |
| 7 | 데이터 export 제한·유료 티어 게이팅 | 모델 기준 ROAS CSV 무제한 내보내기 + 감사로그 CSV export | 락인 없는 데이터 휴대성, 대행사 핸드오프 용이 | △ | ☆ |
| 8 | 무거운 클라우드 의존, 오프라인 시연 불가 | 키 부재 시 mock fallback·localStorage 영속(오프라인 데모) | 영업·심사 자리에서 즉시 시연 | △ | ☆ |

**표 5-6b. 데이터·통합 차별점 (7)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 9 | 매체 비용·전환 데이터 분리(어트리뷰션 도구는 비용 미연동) | 매체 *광고비*와 *기여 매출*을 단일 테이블에 통합 | mROAS(기여매출/광고비) 매체별 직접 산출 | ○ | ★ |
| 10 | 자사몰 통계와 매체 콘솔 숫자 불일치 방치 | reconciliation 설계·허용 오차 ±5% 고지(§2.4·§18) | "숫자가 어긋나면 즉시 이탈"(§구매동인 ④) 위협 완화 | ○ | ★ |
| 11 | 단일 브랜드 뷰만 제공 | 다중 브랜드(오늘의무드·Beanly·Cleanwave) 테넌트 전환 | 멀티 브랜드 운영사·대행사 1계정 관리 | ○ | ☆ |
| 12 | 채널·매체 라벨 비표준(영문 고정) | 한글 매체명·organic/direct 정규화 매핑 | 한국 마케터 인지 부하 감소 | △ | ☆ |
| 13 | 주문/취소 이벤트 구분 없이 매출만 집계 | webhook order/cancel 이벤트 타입 분리 인입 | 취소 반영된 *순* 기여 매출 | ○ | ☆ |
| 14 | CSV 입력 포맷 강제·매핑 수작업 | CSV 광고비 업로드 → 매체 자동 매칭 적용 | 비-API 매체도 수기 1회 업로드로 반영 | △ | ☆ |
| 15 | 데이터 변경 추적 부재 | 모든 데이터·설정 변경 감사로그 기록(actor·brand·time) | 누가 무엇을 바꿨는지 추적, 협업 신뢰 | ○ | ☆ |

**표 5-6c. 어트리뷰션 모델 차별점 (8)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 16 | GA4·매체 콘솔은 Last-Click 단일(과투자 유발) | 6개 모델(last/first/linear/time-decay/markov/shapley) 즉시 전환 | 모델 간 기여 차이를 같은 화면서 비교(§2.5) | ○ | ★ |
| 17 | 멀티터치 미지원(엑셀·자사몰 통계) | **Markov 제거효과(removal effect)** 실 계산 기여 배분 | 단일 매체가 못 주는 크로스매체 기여 | ○ | ★ |
| 18 | Shapley 미제공·블랙박스 | **Shapley value**(순열 표본 평균) 기여 배분 실 구현 | 게임이론 기반 공정 기여, 모델 신뢰 | ○ | ☆ |
| 19 | 모델 전환 시 의사결정 변화 비가시 | Last-Click ↔ Markov 기여 차이 → 예산 액션 변화 시연 | "무엇을 다르게 결정하는가" 입증(§2.5) | ○ | ★ |
| 20 | 기여 경로(여정) 탐색 불가 | 사용자 여정 시퀀스(seq) 기반 경로별 전환확률 산출 | 어떤 경로가 전환을 만드는지 가시화 | ○ | ☆ |
| 21 | 어트리뷰션이 CRM 부가기능(데이터라이즈) | 어트리뷰션 *전담* 제품의 모델 깊이(§5.4) | add-on 대비 UX·모델 정교함 | △ | ☆ |
| 22 | 모델 결과를 URL로 공유 불가 | `#view?model=&brand=&role=` 딥링크 상태 복원 | 팀에 특정 모델 뷰 링크로 공유 | △ | ☆ |
| 23 | 시간감쇠 가중 고정·조정 불가 | time-decay 지수 가중(2^(i/n)) 명시 구현 | 최근 터치 가중 로직 투명 | △ | ☆ |

**표 5-6d. 운영·자동화 차별점 (8)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 24 | 예산 재배분은 수기 판단 | **예산 재배분 추천**(mROAS 균등화 휴리스틱, sqrt 감쇠) 실 계산 | 효율 매체로 이동 시 예상 ROAS lift 제시 | ○ | ★ |
| 25 | 이상 징후를 사람이 눈으로 발견 | **z-score 이상탐지**(\|z\|≥1.8 경고·≥2.4 심각) 자동 플래그 | ROAS 급락 매체를 자동 포착, 손실 조기 차단 | ○ | ★ |
| 26 | 리포트 수작업 작성·발송 | 슬랙·이메일·카카오 알림톡 자동 리포트 스케줄러 | 매일/매주 ROAS 요약 자동 전달 | ○ | ☆ |
| 27 | RFM 세그먼트 수동 분류 | R/F/M 5분위 채점 → 세그먼트 자동 산출 + 액션 추천 | VIP·위험·충성별 다음 액션 제안 | ○ | ☆ |
| 28 | 세그먼트→캠페인 연결 끊김 | 세그먼트 액션 버튼 → 발송 이력(sendLog) 기록 | 세그먼트에서 바로 캠페인 발송(mock) | △ | ☆ |
| 29 | 리포트 미리보기 부재 | 발송 전 리포트 텍스트 미리보기 렌더 | 발송 내용 사전 확인 | △ | ☆ |
| 30 | 발송 채널 단일(이메일만) | 이메일·Slack·알림톡 3채널 선택 | 팀 워크플로에 맞는 채널 | △ | ☆ |
| 31 | 코호트 리텐션 미제공 | 월별 코호트 × M0~M5 잔존 매트릭스 | 리텐션 추세로 LTV 가설 점검 | ○ | ☆ |

**표 5-6e. 가격·비즈니스 모델 차별점 (6)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 32 | Segment $120~/Northbeam $1,500~/Triple Whale $549~[^12][^13] | SMB 진입가 **월 19~99만원** 명시 | SMB 지불의사 내 진입, 견적·협상 0 | △ | ★ |
| 33 | mParticle ACV $156k+·무료티어 없음·6자리 연계약[^14] | 월 단위 구독·연 약정 강요 없음 | 소규모 브랜드도 부담 없이 시작 | △ | ★ |
| 34 | 엔터 기능 번들로 과대 가격 | 단일 제품 저비용 COGS 구조(§6.4) | 가격 인하 여력 = 가격 해자 지속 | ○ | ☆ |
| 35 | 가격 비공개(견적제) | 가격 공개·예측 가능 | 도입 검토 마찰 감소 | △ | ☆ |
| 36 | 광고비 비례 과금(대행사 10~20%, §5.3) | 정액 구독(광고비 증가와 비용 디커플링) | 광고비 늘어도 비용 고정, 단위경제성 개선 | ○ | ☆ |
| 37 | GMV·MTU 연동 과금(Triple Whale·Segment) | 사용량 게이팅 최소화(데모 무제한 export) | 성장해도 비용 폭증 없음 | △ | ☆ |

**표 5-6f. GTM·채널 차별점 (5)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 38 | 대행사를 경쟁자로 직접 영업(페인 약한 세그먼트, §5.3) | **화이트라벨로 대행사를 리셀러로 흡수**(§7) | 페인 약한 브랜드를 채널 우회로 확보 | ○ | ☆ |
| 39 | 글로벌 도구는 한글 CS·온보딩 부재 | 한국어 UI·CS·1-click 온보딩(§2.4) | 도입 장벽·러닝커브 감소 | △ | ★ |
| 40 | 카페24·아임웹 1-click 연동 부재(§5.1) | 국내 자사몰 1-click 연동 지향 | 설치 마찰 최소, TTV 단축 | ○ | ★ |
| 41 | 적합/부적합 세그먼트 미구분 영업 | 매체≥3·월광고비≥1천만 적합 모수로 타깃 좁힘(§3.2) | 약한 구매동인 세그먼트 회피, CAC 효율 | ○ | ☆ |
| 42 | 데이터라이즈 비경합 풀 미식별 | 어트리뷰션 needs 미충족 세그먼트 분리 침투(§5.4) | 정면 충돌 회피한 진입 경로 | △ | ☆ |

**표 5-6g. 네트워크효과·전환비용(Moat) 차별점 (4)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 43 | 선점 lock-in 미축적(신규 진입자) | 선점 속도(§15)로 임계 고객·데이터 누적 → 전환비용 강화 | 시간 경과로 강해지는 비가격 해자(§4.5·§5.5) | ◎ | ☆ |
| 44 | 매체 연동 = 단순 커넥터로 인식 | 데이터 정합 정확도·운영 노하우 축적(§4.6) | 복제돼도 남는 측정 가능 차별(매칭률) | ○ | ☆ |
| 45 | 어트리뷰션 결과를 데이터 학습에 미환류 | 누적 여정 데이터로 모델·휴리스틱 튜닝 여지 | 데이터 많을수록 추천 품질↑(약한 데이터 효과) | ○ | ☆ |
| 46 | 멀티 브랜드 운영사 lock-in 부재 | 다중 테넌트·감사로그로 조직 단위 정착 | 팀·브랜드 늘수록 이탈 비용 증가 | ○ | ☆ |

**표 5-6h. UX·신뢰·거버넌스 차별점 (6)**

| # | 경쟁사 현황 | 우리 차별점 | 고객 가치 | 영속성 | 구매동인 |
|:---:|:---|:---|:---|:---:|:---:|
| 47 | 권한 분기 없이 단일 계정 공유 | 오너/마케터/뷰어 3역할 권한 분기(can(k) 게이팅) | 뷰어는 열람만, 오너만 OAuth·예산 적용 | ○ | ☆ |
| 48 | 권한 매트릭스 비가시 | 역할×기능 권한 매트릭스 화면 노출 | 누가 무엇을 할 수 있는지 투명 | △ | ☆ |
| 49 | 5분 안에 의사결정 못 내림(콘솔 산재) | 단일 대시보드 5분 예산 의사결정(§2.5·구매동인) | 매주 반드시 내리는 결정의 시간·정확도 | ○ | ★ |
| 50 | 정합 오차를 숨기거나 과신 유도 | 허용 오차 명시·정직 고지(±5%) | 과신으로 인한 신뢰 붕괴 방지 | △ | ★ |
| 51 | 에러·빈 상태 방치(빈 차트) | 빈 상태 안내 문구·실패 모드 검수(§반응형 검수) | 신뢰 가는 초기 사용 경험 | △ | ☆ |
| 52 | 한국 D2C 맥락(네이버·카카오·쿠팡) 미반영 UX | 한국 매체·자사몰 워크플로에 맞춘 화면 흐름 | 현지 마케터 멘탈모델 정합 | △ | ☆ |

> **요약** — 총 **52개 차별점**을 8개 카테고리로 도출했다. 이 중 **구매동인 ★(must-have급)** 으로 분류한 것은 §구매동인 논증에서 *적합 세그먼트(§3.2) 한정 강한 must-have*로 재검증된다. 영속성 ◎/○ 항목(특히 #43 전환비용)이 §5.5의 "6개월 일시 우위 → 영속 우위" 전환 경로의 실체다. 나머지 △ 항목은 *현 시점 우위*임을 정직히 표기했다 — 50개를 채우기 위한 억지 항목이 아니라, 모두 §2~§5 본문 또는 데모(`projects/cdp-dashboard`)에서 실제로 구현·논증된 축이다.

---

## 차별화 기술의 구매동인 논증

> §5는 차별점(한국 매체 1급 연동·멀티터치 어트리뷰션·SMB 가격)을 *나열·비교*했다. 본 절은 그 차별점이 **고객의 실제 구매·사용 결정을 얼마나 크게 움직이는지**를 별도로 논증한다. 핵심 차별 기술인 **「멀티터치(Markov) 어트리뷰션 기반 5분 예산 의사결정」**(§2.5·§4.2 JTBD) 하나에 집중해, ①must/nice 분류 → ②가치 정량화 → ③외부 근거 → ④반증 직시 순으로 따진다. 흩어져 있던 JTBD(§4.2)·ROI(§17)·무료대안 경쟁(§5.3)·Last↔Markov 액션변화(§2.5)를 여기서 하나의 구매동인 논증으로 통합한다.

### ① 구매동인 가설 — must-have인가 nice-to-have인가

본 기술 차별점이 건드리는 고객(D2C 사장·마케터)의 핵심 의사결정 요인은 JTBD 언어로 **"이번 주 어느 매체에 광고비를 더 넣을지 5분 안에 데이터로 결정"**(§4.2)이다. 이 Job의 충족 여부로 분류한다.

| 분류 축 | 판정 | 근거 |
|:---|:---:|:---|
| 광고비를 *집행하는* 모든 D2C에게 | **must-have** | 광고비 재배분은 매주 반드시 내려야 하는 의사결정이고, 틀리면 직접 돈이 샌다(§17 절감액). "안 할 수 없는" 결정이다 |
| 매체 ≥3 동시집행·월광고비 ≥1천만 *적합* 세그먼트(§3.2) | **강한 must-have** | 매체가 많을수록 콘솔 합산의 중복집계 오류가 커지고(§1.2), Last-Click 과투자 손실이 누적된다 |
| 매체 1~2개·소액 광고 브랜드 | nice-to-have | GA4·자사몰 통계로 '충분히 좋게' 대체 가능 → 본 사업 타깃에서 제외(§3.2 모수 좁힘과 정합) |

**정직한 단서**: 차별점이 must-have가 되는 것은 *적합 세그먼트 안에서만*이다. 소상공인 D2C 다수는 GA4(0원)로 "충분히 좋은(good enough)" 의사결정을 한다고 *느끼며*(실제로 멀티터치가 아니어도 매주 돈은 쓴다), 이들에게 본 기술은 nice-to-have에 그친다. 따라서 본 절의 must-have 주장은 **§3.2에서 좁힌 적합 모수(약 2만)에 한정**해 성립하며, 그 밖에선 약한 구매동인임을 인정한다(이 한계가 ④ 반증의 출발점이다).

### ② 가치 정량화 — 고객 언어 수치 · 전환비용 대비 10배 규칙

차별점이 만드는 가치를 고객이 체감하는 두 축(시간·돈)으로 환산한다. 모두 §17·§1.2에서 이미 제시한 수치의 재사용이다.

| 가치 축 | 차별점이 만드는 변화 | 수치 | 출처 |
|:---|:---|:---|:---|
| 의사결정 시간 | 5개 콘솔 수기 합산 → 단일 대시보드 자동화 | **−약 4시간/주(≈ −16시간/월)** | §1.2 [내부 인터뷰 추정·공식 2차 출처 부재] |
| 광고비 절감(예산 재배분) | Last-Click 과투자 매체 → 효율 매체 재배분 | 월 광고비 3천만 기준 **−약 210만원/월**, ROI 약 4.3배(신생 3.7배·대형 7.1배) | §17 [추정·미검증] |

**10배 규칙 판정** — 차별점이 *전환 마찰·무료 대안*을 넘으려면 가치가 그 비용의 한 자릿수 배수를 넘어야 한다.

- **vs 구독료 전환비용**: 월 절감 210만 vs 구독 49만 = **약 4.3배[추정]**. 10배엔 못 미치나, 도입 마찰이 낮은 SaaS(1-click 설치·TTV ≤1영업일, §2.4)에서는 4배대도 전환을 정당화하는 구간이다. 단 이 4.3배는 절감률 7%[추정] 가정에 전적으로 의존하므로 §19 파일럿 실측 전까지 *가설*이다.
- **vs GA4(0원) 무료 대안**: 무료 대안의 전환비용은 '가격 0'이 아니라 *멀티터치 부재로 인한 Last-Click 과투자 손실 + 주 4시간 수작업*이다. 이 숨은 비용(시간 −16시간/월 + 잠재 과투자)이 월 19만원(Starter) 구독을 정당화하는지가 실제 구매 임계이며, **적합 세그먼트에서만 넘는다**(①과 정합).

> 위 수치는 모두 [추정]이며 공식 수치와 혼용하지 않는다. 시간 −4시간/주는 내부 인터뷰 추정(§1.2), 절감액·ROI 배수는 §17 가정값이다. 실측 검증 경로는 §19(geo-lift/A·B 파일럿)에 둔다.

### ③ 외부 근거 연결 — 자체 추정과 검증 수치 구분

| 주장 | 근거 유형 | 출처 |
|:---|:---|:---|
| 한국 광고비가 5개 콘솔에 격리돼 통합 needs가 실재 | **외부 공식** | 네이버 검색 4.17조[^9]·카카오 톡비즈 광고형 분기 3,212억[^10]·쿠팡 리테일미디어[^11] |
| 글로벌 도구는 SMB 지불의사를 상회(가격이 구매 장벽) | **외부 공식** | Segment $120~·견적[^12], Northbeam $1,500~·Triple Whale $549~[^13] |
| 동일 채널에서 SMB가 유료 SaaS로 전환한 선례 존재 | **외부 공식(타사 실적)** | 데이터라이즈 유료 1,000곳+[^16] (본 사업 수요와는 구분) |
| 쿠키리스로 매체 콘솔 어트리뷰션 신뢰도 하락 → 1st-party 수요 | **외부 공식** | Chrome 철회·Safari/Firefox 차단 유지[^18] |
| 주 4시간 수작업·7% 절감률·ROI 4.3배 | **자체 추정 [추정·미검증]** | §1.2·§17 (§19 파일럿으로 실측 예정) |

자체 추정(시간·절감률·ROI 배수)은 모두 `[추정]`으로 표기하고, 검증된 외부 수치(시장규모·경쟁사 가격·타사 실적)와 한 문장에 섞지 않는다. 구매동인의 *방향*(통합·멀티터치 needs 실재, 가격이 글로벌 도구 진입 장벽)은 외부 공식 근거가 받치고, 구매동인의 *크기*(절감액·ROI)는 아직 자체 가설이다.

### ④ 반증 직시 — 차별점에도 *안 사는/이탈하는* 이유와 대응

차별점이 강해 보여도 고객이 사지 않거나 이탈하는 경로를 정직하게 적고, 그 위협을 어떻게 넘는지 답한다.

| 반증(왜 안 사나/이탈하나) | 강도 | 대응 |
|:---|:---:|:---|
| **GA4(0원) '충분히 좋다'** — 가장 강력한 경쟁은 '아무것도 안 사는 것' | 高 | §5.3대로 *적합 세그먼트*만 겨냥. 매체 비용 통합·한국매체 1급·멀티터치로 GA4가 못 주는 가치 입증. 19만원 점프 수용성은 §6.1 벤치마크·§17 ROI로 보강 |
| **대행사 사용 브랜드는 페인이 약함** — 직접 구매동인 낮음 | 中~高 | 직접 영업 대신 §7 화이트라벨로 대행사를 리셀러로 흡수(우회) |
| **숫자가 GA4·매체 콘솔과 어긋나면 즉시 이탈** | 高 | §2.4 정합성 UX(허용 오차 ±5%·고지)·§18 reconciliation·§19 매칭률 ≥80% |
| **플랫폼 disintermediation**(매체사 1st-party 통합 리포트)·데이터라이즈 add-on | 中~高 | §4.5 전환비용·선점 속도, §5.4 전담 제품 깊이로 방어 |
| **절감률 7% 가정이 실측에서 무너지면 ROI 논거 붕괴** | 中 | §19 geo-lift/A·B 파일럿으로 실측. 무너지면 구매동인을 '시간 절감(−16시간/월)' 중심으로 재포지셔닝 |

> 정직한 결론: 본 차별점은 **적합 세그먼트(§3.2) 안에서 강한 must-have, 그 밖에선 nice-to-have**다. 가장 큰 위협은 경쟁 SaaS가 아니라 *무료 대안의 관성*이며, 이를 넘는 핵심은 (a)적합 타깃 좁히기 (b)정합 신뢰(매칭률·오차) (c)§19 ROI 실측이다.

### 데모(projects/cdp-dashboard)가 이 구매동인을 시연하는 지점

논증과 산출물의 정합을 위해, 위 구매동인이 데모에서 **실제로 구현·시연**되는 화면을 지목한다.

| 구매동인 요소 | 데모 시연 지점 (cdp-dashboard) |
|:---|:---|
| 멀티터치가 *무엇을 다르게 결정하게 하는가*(§2.5) | **「어트리뷰션」 화면** — 상단 모델 선택 `<select>`(Last Click ↔ Markov 등 6모델)로 모델을 바꾸면, **"Last Click vs Markov 비교" 차트**와 **"모델별 의사결정 차이" 표**(매체별 Last/Markov 기여도·차이)가 즉시 갱신된다 |
| 5분 예산 의사결정(JTBD §4.2) | **「예산 최적화」 화면** — 선택한 어트리뷰션 모델 기여도 기반으로 매체별 **추천 예산·증감·예상 ROAS**를 자동 산출. 모델을 Last→Markov로 바꾸면 추천 예산 배분이 달라지는 것을 실시간 확인 |
| 액션 적용(재배분 실행) | 「예산 최적화」의 **'추천 예산 적용'** 버튼 → 매체별 예산이 갱신되고 대시보드·감사로그(audit)에 반영 |

즉 심사·시연 자리에서 **어트리뷰션 모델을 Last↔Markov로 전환했을 때 예산 재배분 액션이 실제로 바뀌는 화면**을 직접 보여, "멀티터치 어트리뷰션이 구매동인(5분 의사결정)을 실제로 움직인다"는 본 절의 논증을 산출물로 입증한다.
...
| 티어 | 월 가격 | 포함 | 타깃 |
|:---|---:|:---|:---|
| Free / Trial | 0원 | 매체 1개·14일 전체기능 체험(Markov 포함) → 매체 ≥3·Markov 상시 사용이 유료 트리거 | PLG 진입(§7 GTM 깔때기 입구) |
...
| 채널 | 신규고객 믹스 비중 [추정] | 수수료/비용 | 예상 전환율 [추정] | 추정 CAC | 순ARPU 영향 |
|:---|:---:|:---|:---:|:---:|:---|
| 마켓플레이스(카페24·아임웹 앱스토어) | 40% | 레브셰어 20~30%[재확인 필요] | 무료→유료 5~10% | 약 30만 | ARPU의 20~30% 차감(COGS 반영) |
| 콘텐츠/SEO·컨퍼런스 | 40% | 제작·운영비 | 1~3% | 약 80만 | 차감 없음 |
| 대행사 화이트라벨 | 20% | 리셀러 수수료 20~30% | — | 약 50만(대행사가 온보딩) | 순ARPU = ARPU×(1−리셀러%) |
| **블렌디드(가중평균)** | 100% | — | — | **약 54만(≈55만)** | 0.4×30+0.4×80+0.2×50 / GM 75% 산정에 반영 |
...
**첫 100곳 / 첫 1,000곳 확보 — 실행 단위 분해 (목표 숫자가 아니라 '어디서·어떻게')**

> 추상 목표("바이럴 확산")가 아니라, 구체 채널·콘텐츠·1순위 파트너를 실행 단위로 못박는다. 모든 수치는 [추정] 설계값이며 실측은 §1.5·§15 KPI로 검증한다.

| 구간 | 핵심 채널·전술 (실행 단위) | 정량 가설 [추정] | 검증/근거 |
|:---|:---|:---|:---|
| **첫 1~10곳 (수동 영업)** | 창업팀 직접 콜드아웃리치 + 무료 파일럿. 타깃 리스트 = 카페24·아임웹 앱스토어 '마케팅/광고·통계분석' 카테고리 상위 노출 브랜드 중 *매체 ≥3·월광고비 ≥1천만* 필터 | LOI 5곳·유료 파일럿 1곳(§1.5 M3) | §1.5 트랙션 게이트 |
| **첫 100곳 (마켓플레이스 입점)** | ① 카페24 '마케팅' + 아임웹 '데이터/통계' 앱스토어 카테고리에 무료 플러그인 등록 → 카테고리 내 상위 노출 ② "글로벌 도구 대신 로아스KR" 비교 SEO 콘텐츠 10편 ③ 디지털마케팅 컨퍼런스 1회(예: 이커머스/그로스 밋업) 부스·세션 발표 | 설치 300~800(웨이팅리스트)→ 활성화 40%→ 유료 100곳(M12 목표) | §1.5 트랙션·§7 PLG 깔때기 |
| **첫 1,000곳 (대행사 화이트라벨 레버리지)** | 광고 대행사 **선도 1곳**을 화이트라벨 리셀러로 확보 → 그 대행사의 관리 브랜드 수십~수백곳을 묶음 온보딩. 대행사 1곳이 100곳을 데려오면 직접영업 대비 CAC 급감 | 대행사 5~10곳 × 평균 50~100 브랜드 ≈ 500~1,000곳(Y2~Y3) | §6.4 화이트라벨 채널·§7 Series A |

> 핵심 레버: **첫 100곳은 마켓플레이스 카테고리 노출(낮은 CAC)**, **첫 1,000곳은 대행사 화이트라벨(대행사 1곳=수십 브랜드 묶음)**. 직접 1,000곳 영업이 아니라 대행사를 통한 묶음 획득이 단위경제(§6.4 화이트라벨 CAC 낮음)와 정합한다.
...
> **두 'BEP/회수' 지표 구분(혼동 방지)** — ① **고객 단위 CAC 회수기간(payback)** = CAC ÷ 월 기여이익 = 55만 ÷ 37.5만 ≈ **1.5개월**(§6.3, 고객 1곳이 획득비를 회수하는 데 걸리는 기간). ② **회사 전체 월 손익분기(BEP)** = 월 고정비 ÷ 월 기여이익 ≈ 400 브랜드, **Y2 말~Y3 초** 도달(고정비까지 덮어 월간 흑자 전환하는 시점). 둘은 다른 개념이며 본 제안서에서 각각 1.5개월·Y2말~Y3초로 일관 사용한다.
...
> **출처 정직성 표기 (현재/목표 = 19 / 1,000).** 본 제안서가 현재 본문에서 인용·검증한 1차/공신력 출처는 **19건**([^1]~[^18], 일부 ^3a·^7b 분기 출처 포함)이다. 키트 권위 기준([k-startup-template §2.1](../../k-startup-template/CLAUDE.md))의 목표치는 **1,000건**이며, 현재는 목표 대비 일부만 충족한 상태다. 미달분은 향후 `5_research/` 출처 확장으로 채운다. **날조·유령 인용(존재하지 않는 URL/논문)·동일 출처 분할 부풀리기는 하지 않는다** — 정직성이 수량보다 우선([§데이터 정직성 선언](#데이터-정직성-선언)).
>
